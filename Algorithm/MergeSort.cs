    ...
    public class MergeSort<T> : AlgorithmBase<T> where T : IComparable
    {
        ...
        private List<T> Merge(List<T> left, List<T> right)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                {
                    //there elements which uder pointers

                    if (Compare(left[leftPointer], right[rightPointer]) == -1) //if left < right
                    {
                ...
                {
                    //there other elements in Lists

                    //check second condition again. (if first condition false, second condition not checking)
